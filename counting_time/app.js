...
// 事件绑定
document.getElementById('addBtn').addEventListener('click', handleAddCountdown)
...
function sortCountdowns() {
    ...
    return [...window.countdowns].sort((a, b) => {
        ...
        // 排序逻辑：未来日期正序，过去日期倒序
        return aDiff >= 0 && bDiff >= 0 ? aDiff - bDiff :
               aDiff >= 0 ? -1 :
               bDiff >= 0 ? 1 :
...
function getDaysText(targetDateStr) {
    ...
    return targetStart >= nowStart ? `剩余 ${days} 天` : `已过去 ${days} 天`
}
...
function generateIssueHtml(item) {
    return item.issue ? 
        `<a href="https://redmine.licsber.site/issues/${item.issue}" class="issue-link" target="_blank">${item.issue}</a>` : 
        `<span class="issue-plain">无</span>`;
}
...
function handleImport(e) {
    ...
    reader.onload = (event) => {
        try {
            ...
            if (!Array.isArray(importedData)) throw new Error('无效的JSON格式：需要数组类型')
            // 关键修复：为每个导入条目生成唯一id（时间戳+索引）
            window.countdowns = importedData.map((item, index) => ({
                ...item,
                id: Date.now() + index // 确保每个条目的id唯一
            }))
            saveToLocalStorage()
            renderList()
